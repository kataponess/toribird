...
class DisplayParrotsController extends Controller
{
	public function display()
	{
		for ($i = 1; $i <= (\App\Zooname::count()); $i++) {
			$prefecturename = \App\Zooname::find($i)->prefecture->prefecturename;
			$zooname = \App\Zooname::find($i)->zooname;

			$parrotsTable[$prefecturename][$zooname] = \App\Zooname::find($i)->parrots->sortby('parrotname_id');
		}
